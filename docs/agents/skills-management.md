...
在“扩展 → Skills”中点击技能卡片打开预览，有管理权限的个人或共享技能会显示“修改名称”。保存后，技能卡片、选择器和导出文件中的展示名会更新。内部 slug、目录名、`@` 引用、依赖及已有绑定保持不变。

内置技能由系统发布包维护，不能在技能页改名；仅有使用权限的共享技能也不会显示改名入口。个人技能只能由所属用户修改，管理员不能通过个人技能接口改动其他用户的文件。

### 导出个人 Skill

进入“扩展 → Skills”的“个人技能”区域，点击技能卡片打开预览，再点击“导出 ZIP”。浏览器下载以技能 slug 命名的压缩包，包内以 slug 为根目录，包含 `SKILL.md`、脚本、参考资料和其他配套文件；空目录和二进制文件也会保留。可以通过上述上传安装流程重新导入该 ZIP；当导入内容的 slug 与当前用户已有个人 Skill 相同时，确认页会标记“将更新”，确认后原子替换原技能目录。

登录用户只能导出自己的个人 Skill，管理员身份也不能用该入口读取其他用户的个人目录。导出拒绝符号链接和特殊文件；文件缺失或读取失败时显示错误，不下载不完整归档。归档不附加平台账号或共享权限元数据。
