...
- Office：`.doc`、`.docx`、`.wps`、`.pptx`、`.xls`、`.xlsx`；
- PDF：`.pdf`；
- 图片：`.jpg`、`.jpeg`、`.png`、`.bmp`、`.tiff`、`.tif`；
- ZIP：压缩包内必须包含 UTF-8 编码的 `.md` 文件。

旧版 `.doc` 和 `.wps` 使用运行环境中的 LibreOffice 转为临时 `.docx`，再提取正文和表格，不覆盖原文件。WPS 兼容性取决于具体文件格式和转换器，不保证所有历史专有格式都可读取。损坏或加密文件可能转换失败；扫描图片中的文字仍需要 OCR。
