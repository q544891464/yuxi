...
人物布局进一步固定为两行 Grid（`minmax(0, 1fr) auto`）：第一行容纳消息或欢迎页，第二行为 composerZone。装饰 slot 直接归属 composerZone，165×165px 人物框仅相对该区域定位；区域顶部真实预留 165px，图片使用 contain 和 left bottom，并且人物框及图片均禁用 pointer events。素材 alpha 边界检查未发现巨大透明留白。1280×720、1920×1080 本地构建页面的模拟长表格滚动到底部验证了边界分离、定位父级和固定尺寸；模拟编辑区输入通过，不代表真实模型发送链路验证。
